...
def main(args):
    ...
    #train_data_file = os.path.join(args.data_dir, "train_data.json")
    #valid_data_file = os.path.join(args.data_dir, "val_data.json")

    train_data = read_json(train_data_file)
    valid_data = read_json(valid_data_file)

    # tokenizer
    #tokenizer = T5Tokenizer.from_pretrained(args.pretrained_model_name_or_path, legacy=False)
    tokenizer = MT5Tokenizer.from_pretrained(args.pretrained_model_name_or_path, legacy=False)
    ...
    for cls in classes_map.keys():
        for cls in classes_map.keys():
            ...
            if not labels_id:
                raise ValueError(f"Label {cls} is empty after encoding.")
            elif len(labels_id) > 1:
                raise ValueError(f"Label {cls} encodes to multiple tokens: {labels_id}.")
            else:
    ...
    model = MT5ForConditionalGeneration.from_pretrained(args.pretrained_model_name_or_path)
